...
namespace ChromaWrapper.Tests
{
    ...
    {
        private static readonly Regex _rxUrl = new Regex(@"^https\://github.com/poveden/ChromaWrapper/(?:releases/tag/(?<tag>.+?)|compare/(?<tag1>.+?)\.{3}(?<tag2>.+?))$");
        ...
        [Fact]
        public void MostRecentReleaseMatchesManifestVersion()
        {
            const string ProjectPath = @"src\ChromaWrapper.csproj";
            const string XPath = "/Project/PropertyGroup/Version";
            const string VersionExpr = @"^\d+\.\d+\.\d+$";

            string solutionDir = MetaTestsCommon.GetSolutionDirectory();

            string path = Path.Combine(solutionDir, ProjectPath);
            var project = XDocument.Load(path);
            var version = project.XPathSelectElement(XPath);
            Assert.NotNull(version);
            Assert.Matches(VersionExpr, version!.Value);

            var entry = _entries.First(x => x.Valid && x.Released);

            Assert.Equal(version.Value, entry.ReleaseVersion);
        }
